...
namespace Jellyfin.Plugin.JellyfinEnhanced.Controllers
{
    ...
    public class JellyfinEnhancedController : ControllerBase
    {
        ...
        // ─── External-client API ──────────────────────────────────────────
        // The endpoints below are designed for native clients (Streamyfin,
        // Findroid, Swiftfin, custom apps) that bypass Jellyfin's MVC
        // pipeline and therefore don't go through SpoilerFieldStripFilter
        // automatically. They expose enough state for a client to apply
        // the same strip/blur rules locally.
        //
        // All response shapes are versioned via the `apiVersion` field on
        // /info — bumping the integer is a breaking change. Adding fields
        // is non-breaking (clients should ignore unknown fields).
        //
        // Doc: SPOILER_BLUR_API.md at repo root.

        // Capability-discovery endpoint. No-auth so a client can probe
        // before the user has even authenticated. Returns plugin version,
        // API version, and the server-wide enable bit.
        [HttpGet("spoiler-blur/info")]
        [AllowAnonymous]
        [Produces("application/json")]
        public IActionResult GetSpoilerBlurApiInfo()
        {
            ...
            return Ok(new
            {
                ...
                available = true,
                serverEnabled = cfg?.SpoilerBlurEnabled == true,
            });
        }

        // Server-wide spoiler-blur config (admin-set, user-readable). Lets
        // a client mirror the strip rules client-side without round-tripping
        // every item through the field-strip MVC filter.
        [HttpGet("spoiler-blur/config")]
        [Authorize]
        [Produces("application/json")]
        public IActionResult GetSpoilerBlurConfig()
        {
            var cfg = JellyfinEnhanced.Instance?.Configuration;
            if (cfg == null) return NotFound();
            return Ok(new
            {
                enabled = cfg.SpoilerBlurEnabled,
                blurMode = cfg.SpoilerBlurMode ?? "blur",
                blurArtwork = cfg.SpoilerBlurArtwork,
                blurIntensity = cfg.SpoilerBlurIntensity,
                stripOverview = cfg.SpoilerStripOverview,
                stripTags = cfg.SpoilerStripTags,
                stripChapters = cfg.SpoilerStripChapters,
                stripTaglines = cfg.SpoilerStripTaglines,
                stripCommunityRating = cfg.SpoilerStripCommunityRating,
                stripCriticRating = cfg.SpoilerStripCriticRating,
                stripPremiereDate = cfg.SpoilerStripPremiereDate,
                replaceTitle = cfg.SpoilerReplaceTitle,
                stripCast = cfg.SpoilerStripCast,
                stripCastMode = cfg.SpoilerStripCastMode ?? "GuestStars",
                stripReviews = cfg.SpoilerStripReviews,
                overviewPlaceholder = cfg.SpoilerOverviewPlaceholder ?? "Spoiler mode activated",
            });
        }

        // Symmetric to GetSpoilerBlurSeries: returns the current user's
        // movie spoiler list as a map of movieId → entry. Same strict-
        // read corruption-detection (503 on parse failure).
        [HttpGet("spoiler-blur/movies")]
        [Authorize]
        [Produces("application/json")]
        public IActionResult GetSpoilerBlurMovies()
        {
            var userId = UserHelper.GetCurrentUserId(User);
            if (userId == null || userId == Guid.Empty) return Forbid();
            var userKey = userId.Value.ToString("N");
            ...
            if (!_userConfigurationManager.UserConfigurationExists(userKey, fileName))
            {
                return Ok(new { Movies = new Dictionary<string, SpoilerBlurMovieEntry>() });
            }
            try
            {
                var state = _userConfigurationManager.GetUserConfigurationStrict<UserSpoilerBlur>(
                    userKey, fileName);
                return Ok(new { state.Movies });
            }
            catch (InvalidDataException strictEx)
            {
                _logger.Warning($"spoilerblur.json corrupt for {ResolveUserDisplay(userKey)} (backed up): {strictEx.Message}");
        ...
        // All-in-one combined-state convenience endpoint. Cuts the cold-
        // start round-trip count for clients from 3 (config + series +
        // movies) to 1.
        [HttpGet("spoiler-blur/state")]
        [Authorize]
        [Produces("application/json")]
        public IActionResult GetSpoilerBlurState()
        {
            var cfg = JellyfinEnhanced.Instance?.Configuration;
            var userId = UserHelper.GetCurrentUserId(User);
            if (userId == null || userId == Guid.Empty) return Forbid();
            ...
            UserSpoilerBlur state = new UserSpoilerBlur();
            if (_userConfigurationManager.UserConfigurationExists(userKey, fileName))
            {
                try
                {
                    state = _userConfigurationManager.GetUserConfigurationStrict<UserSpoilerBlur>(
                        userKey, fileName);
                }
                catch (InvalidDataException strictEx)
                {
            ...
            return Ok(new
            {
                config = new
                {
                    enabled = cfg?.SpoilerBlurEnabled == true,
                    blurMode = cfg?.SpoilerBlurMode ?? "blur",
                    blurArtwork = cfg?.SpoilerBlurArtwork == true,
                    blurIntensity = cfg?.SpoilerBlurIntensity ?? 40,
                    stripOverview = cfg?.SpoilerStripOverview == true,
                    stripTags = cfg?.SpoilerStripTags == true,
                    stripChapters = cfg?.SpoilerStripChapters == true,
                    stripTaglines = cfg?.SpoilerStripTaglines == true,
                    stripCommunityRating = cfg?.SpoilerStripCommunityRating == true,
                    stripCriticRating = cfg?.SpoilerStripCriticRating == true,
                    stripPremiereDate = cfg?.SpoilerStripPremiereDate == true,
                    replaceTitle = cfg?.SpoilerReplaceTitle == true,
                    stripCast = cfg?.SpoilerStripCast == true,
                    stripCastMode = cfg?.SpoilerStripCastMode ?? "GuestStars",
                    stripReviews = cfg?.SpoilerStripReviews != false, // default true
                    overviewPlaceholder = cfg?.SpoilerOverviewPlaceholder ?? "Spoiler mode activated",
                },
                series = state.Series,
                movies = state.Movies,
        ...
        public class SpoilerEvaluateRequest
        {
            public string[]? ItemIds { get; set; }
        }

        // Per-item evaluation: should this item be blur/stripped, and if so
        // which fields? Designed so a native client can mirror the server-
        // side filter behaviour locally on items it pulled directly via
        // Jellyfin's API (bypassing the JE field-strip MVC filter).
        [HttpGet("spoiler-blur/check/{itemId}")]
        [Authorize]
        [Produces("application/json")]
        public IActionResult CheckSpoilerBlurForItem(string itemId)
        {
            if (!Guid.TryParse(itemId, out var guid)
                && !Guid.TryParseExact(itemId, "N", out guid))
            {
            ...
            var state = LoadSpoilerStateForTagStrip(userId.Value) ?? new UserSpoilerBlur();
            var result = EvaluateItemForSpoiler(guid, state, cfg, jUser);
            if (result == null) return NotFound(new { success = false, message = "Item not found or not accessible." });
            return Ok(result);
        }

        // Batch evaluation. Up to 200 IDs per call so a card-grid render
        // doesn't fan out 200 individual /check requests.
        [HttpPost("spoiler-blur/evaluate")]
        [Authorize]
        [Produces("application/json")]
        public IActionResult EvaluateSpoilerBlurBatch([FromBody] SpoilerEvaluateRequest body)
        {
            if (body?.ItemIds == null || body.ItemIds.Length == 0)
            {
                return BadRequest(new { success = false, message = "itemIds required." });
            }
            if (body.ItemIds.Length > 200)
            {
                return BadRequest(new { success = false, message = "Maximum 200 items per request." });
            }
            var cfg = JellyfinEnhanced.Instance?.Configuration;
            var userId = UserHelper.GetCurrentUserId(User);
            if (userId == null || userId == Guid.Empty) return Forbid();
            ...
            var state = LoadSpoilerStateForTagStrip(userId.Value) ?? new UserSpoilerBlur();
            var results = new Dictionary<string, object?>(body.ItemIds.Length);
            foreach (var idStr in body.ItemIds)
            {
                if (string.IsNullOrEmpty(idStr)) continue;
                if (!Guid.TryParse(idStr, out var guid)
                    && !Guid.TryParseExact(idStr, "N", out guid))
                {
                    results[idStr] = null;
                    continue;
                }
                results[idStr] = EvaluateItemForSpoiler(guid, state, cfg, jUser);
            }
            return Ok(new { items = results });
        }

        // Shared evaluation logic for /check and /evaluate. Returns null
        // when the item isn't found or not accessible to the user.
        private object? EvaluateItemForSpoiler(Guid itemId, UserSpoilerBlur state, PluginConfiguration? cfg, JUser user)
        {
            try
            {
                var item = _libraryManager.GetItemById<MediaBrowser.Controller.Entities.BaseItem>(itemId, user);
                if (item == null) return null;

                var serverEnabled = cfg?.SpoilerBlurEnabled == true;
                string itemType;
                bool inList = false;
                bool watched = false;
                long playbackPositionTicks = 0;
                Guid seriesIdForList = Guid.Empty;
                var fieldsToStrip = new List<string>();
                string? replaceName = null;
                int? seasonNumber = null;
                int? episodeNumber = null;
                var chaptersThatStripName = new List<int>();

                if (item is MediaBrowser.Controller.Entities.TV.Episode ep)
                {
                    itemType = "Episode";
                    seriesIdForList = ep.SeriesId;
                    inList = seriesIdForList != Guid.Empty
                        && state.Series.ContainsKey(seriesIdForList.ToString("N"));
                    var ud = _userDataManager.GetUserData(user, ep);
                    watched = ud?.Played == true;
                    playbackPositionTicks = ud?.PlaybackPositionTicks ?? 0;
                    seasonNumber = ep.ParentIndexNumber;
                    episodeNumber = ep.IndexNumber;
                    if (cfg?.SpoilerReplaceTitle == true && seasonNumber.HasValue && episodeNumber.HasValue)
                    {
                        replaceName = $"Season {seasonNumber.Value}, Episode {episodeNumber.Value}";
                    }
                }
                else if (item is MediaBrowser.Controller.Entities.TV.Season s)
                {
                    itemType = "Season";
                    seriesIdForList = s.SeriesId;
                    inList = seriesIdForList != Guid.Empty
                        && state.Series.ContainsKey(seriesIdForList.ToString("N"));
                    seasonNumber = s.IndexNumber;
                    // Season-level "watched" is treated as "any episode in season played".
                    if (inList && seasonNumber.GetValueOrDefault(int.MaxValue) > 1)
                    {
                        try
                        {
                            foreach (var child in s.GetEpisodes(user, new MediaBrowser.Controller.Dto.DtoOptions(false), shouldIncludeMissingEpisodes: false))
                            {
                                if (child == null) continue;
                                var cud = _userDataManager.GetUserData(user, child);
                                if (cud?.Played == true) { watched = true; break; }
                            }
                        }
                        catch { /* fail-CLOSED: watched stays false */ }
                    }
                    else
                    {
                        // S0 / S1 / non-list always treated as "shown" (no strip).
                        watched = true;
                    }
                    if (cfg?.SpoilerReplaceTitle == true && seasonNumber.HasValue)
                    {
                        replaceName = $"Season {seasonNumber.Value}";
                    }
                }
                else if (item is MediaBrowser.Controller.Entities.Movies.Movie movie)
                {
                    itemType = "Movie";
                    inList = state.Movies.ContainsKey(movie.Id.ToString("N"));
                    var ud = _userDataManager.GetUserData(user, movie);
                    watched = ud?.Played == true;
                    playbackPositionTicks = ud?.PlaybackPositionTicks ?? 0;
                    // R17 carve-out: movie titles are NOT replaced.
                    replaceName = null;

                    // Progressive chapter strip: report chapter indexes
                    // whose Name should be hidden (after playback position).
                    if (inList && !watched && cfg?.SpoilerStripChapters == true)
                    {
                        try
                        {
                            var chapters = _chapterManager.GetChapters(movie.Id);
                            if (chapters != null)
                            {
                                long throughTicks = playbackPositionTicks > 0 ? playbackPositionTicks : 0;
                                for (int i = 0; i < chapters.Count; i++)
                                {
                                    var c = chapters[i];
                                    // Strict-less-than (R17-codex-M1).
                                    if (c.StartPositionTicks >= throughTicks)
                                    {
                                        chaptersThatStripName.Add(i);
                                    }
                                }
                            }
                        }
                        catch { /* defensive — skip chapter info on lookup failure */ }
                    }
                }
                else if (item is MediaBrowser.Controller.Entities.TV.Series ser)
                {
                    itemType = "Series";
                    inList = state.Series.ContainsKey(ser.Id.ToString("N"));
                    // Series itself has no per-watched concept. Strip applies
                    // whenever in-list (R17-Series-cast).
                    watched = false;
                }
                else
                {
                    itemType = item.GetType().Name;
                    // Extras / trailers / videos with SeriesId pointing at a
                    // spoiler-list series — see R10-codex-H + R12-codex-H.
                    var hasSeries = item.GetType().GetProperty("SeriesId");
                    if (hasSeries != null
                        && hasSeries.GetValue(item) is Guid sid
                        && sid != Guid.Empty
                        && state.Series.ContainsKey(sid.ToString("N")))
                    {
                        inList = true;
                        seriesIdForList = sid;
                    }
                ...
                // Decide whether to recommend strip/blur. For Episode/Movie
                // we honor watched state. Series + extras: always strip
                // when in list (no per-item watched semantics there).
                bool shouldBlur = serverEnabled && inList && !watched;
                bool shouldStripMetadata = shouldBlur;
                if (itemType == "Series" || (itemType != "Episode" && itemType != "Movie" && itemType != "Season"))
                {
                    // Series + extras: always-strip-when-in-list.
                    shouldBlur = serverEnabled && inList;
                    shouldStripMetadata = shouldBlur;
                }

                // Field list reflects which strip toggles are on. Clients
                // honor only the ones they care about.
                if (shouldStripMetadata)
                {
                    if (cfg?.SpoilerStripOverview == true) fieldsToStrip.Add("overview");
                    if (cfg?.SpoilerStripTags == true) fieldsToStrip.Add("tags");
                    if (cfg?.SpoilerStripChapters == true) fieldsToStrip.Add("chapters");
                    if (cfg?.SpoilerStripTaglines == true) fieldsToStrip.Add("taglines");
                    if (cfg?.SpoilerStripCommunityRating == true) fieldsToStrip.Add("communityRating");
                    if (cfg?.SpoilerStripCriticRating == true) fieldsToStrip.Add("criticRating");
                    if (cfg?.SpoilerStripPremiereDate == true) fieldsToStrip.Add("premiereDate");
                    if (cfg?.SpoilerStripCast == true) fieldsToStrip.Add("cast");
                    if (cfg?.SpoilerReplaceTitle == true && itemType != "Movie") fieldsToStrip.Add("name");
                    if (cfg?.SpoilerReplaceTitle == true || cfg?.SpoilerStripOverview == true)
                    {
                        // Cross-cutting strip group: paths / streams / external links.
                        fieldsToStrip.Add("path");
                        fieldsToStrip.Add("mediaStreams");
                        fieldsToStrip.Add("mediaSources");
                        fieldsToStrip.Add("remoteTrailers");
                        fieldsToStrip.Add("externalUrls");
                    }
                }
                ...
                return new
                {
                    ...
                    itemType = itemType,
                    inSpoilerList = inList,
                    watched = watched,
                    playbackPositionTicks = playbackPositionTicks,
                    shouldBlur = shouldBlur,
                    shouldStripMetadata = shouldStripMetadata,
                    fieldsToStrip = fieldsToStrip,
                    replaceName = replaceName,
                    placeholder = cfg?.SpoilerOverviewPlaceholder ?? "Spoiler mode activated",
                    seriesId = seriesIdForList == Guid.Empty ? (string?)null : seriesIdForList.ToString("N"),
